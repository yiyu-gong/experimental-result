...
def run_kind2(file_name):
    ...
    kind2_command = f"/usr/bin/time -v ./IC3_UC --enable IC3QE -vv --color false --timeout 500 {file_name}.lus"
    try:
        ...
        pattern_sml_length = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?sml_length\s+:\s+(\d+)')
        pattern_ucl_f_length = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?ucl_f_length\s+:\s+(\d+)')
        pattern_ucl_i_length = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?ucl_i_length\s+:\s+(\d+)')
        pattern_sml_mem = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?sml_mem\s+:\s+([\d.]+)')
        pattern_ucl_f_mem = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?ucl_f_mem\s+:\s+([\d.]+)')
        pattern_ucl_i_mem = re.compile(r'Final statistics:[\s\S]*?\[IC3QE\][\s\S]*?ucl_i_mem\s+:\s+([\d.]+)')
        ...
        match_sml_length = pattern_sml_length.search(output.stdout)
        match_ucl_f_length = pattern_ucl_f_length.search(output.stdout)
        match_ucl_i_length = pattern_ucl_i_length.search(output.stdout)
        match_sml_mem = pattern_sml_mem.search(output.stdout)
        match_ucl_f_mem = pattern_ucl_f_mem.search(output.stdout)
        match_ucl_i_mem = pattern_ucl_i_mem.search(output.stdout)
        ...
        sml_length = match_sml_length.group(1) if match_sml_length else "N/A"
        ucl_f_length = match_ucl_f_length.group(1) if match_ucl_f_length else "N/A"
        ucl_i_length = match_ucl_i_length.group(1) if match_ucl_i_length else "N/A"
        sml_mem = match_sml_mem.group(1) if match_sml_mem else "N/A"
        ucl_f_mem = match_ucl_f_mem.group(1) if match_ucl_f_mem else "N/A"
        ucl_i_mem = match_ucl_i_mem.group(1) if match_ucl_i_mem else "N/A"


        return file_name, result, ic3_k, ic3_total_time, ic3_frame_sizes, ic3_solver, ic3_extra_ucl, ic3_add_ucl, ic3_check_ucl, ic3_add_sml, ic3_extra_sml, ic3_extra_time_c, ic3_extra_time_c_n1, ic3_extra_time_F, ic3_assert_time, ic3_check_sat_time, ic3_get_unsat_core_time, ic3_get_value_time, i_ucl, i_reuse_ucl, f_ucl, f_reuse_ucl, f_sml, f_reuse_sml, sml_length, ucl_f_length, ucl_i_length, sml_mem, ucl_f_mem, ucl_i_mem ,ic3_max_size

    except subprocess.TimeoutExpired:
        # timeout
        return file_name,"timeout","N/A","N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A","N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A","N/A",	"N/A",	"N/A",	"N/A",	"N/A",	"N/A","N/A",	"N/A",	"N/A" , "N/A"
...
# 调用批量运行函数
batch_run_kind2("/experimental-result-main/supplement/benchmark.csv", "IC3_UC_all.csv")
